...
| `dependencies.py --scope core` or `--scope media` | Reports what the scope requires and what it does not, each with its version, status and the work it enables, then names what is missing and what that affects. Installs nothing | Missing dependency is not a quality pass |
